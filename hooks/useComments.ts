...
export const useComments = ({
  ...
  const handleCommentSubmit = useCallback(
    async (name: string, email: string, comment: string) => {
      setLoading(true);
  ...
  const handleCommentLike = useCallback(async (commentId: string) => {
    ...
    try {
      ...
      const result = await client.patch(commentId).inc({ likes: 1 }).commit();
  ...
  const transformedComments = comments.map((comment) => ({
    ...
    _id: comment._id, // Preserve the original Sanity _id
    author: comment.name,
